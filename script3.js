...
function checkAnswer(){
  ...
  if(!userVal)return;
  answered=true;
  const isCorrect=String(userVal).replace(/\s/g,'').toLowerCase()===String(currentQ.ans).replace(/\s/g,'').toLowerCase();
...
function showFeedback(isCorrect,ans){
  if(isCorrect){correct++;totalScore+=getDiffPoints();}
  else{wrong++;}
  updateHUD();

  const fb=document.getElementById('feedbackBar');
  fb.textContent=isCorrect?`✓ Correct! Well done.`:`✗ Wrong. Answer: ${ans}`;
  fb.className='feedback-bar show '+(isCorrect?'correct':'wrong');

  // progress
  const acc=qCount>0?Math.round(correct/qCount*100):0;
  document.getElementById('progressBar').style.width=acc+'%';

  // auto-advance — no manual "Next" click needed
  setTimeout(nextQuestion, isCorrect?900:1800);
}

function getDiffPoints(){return difficulty===0?5:difficulty===1?10:20}
...
function updateHUD(){
  ...
  document.getElementById('pQ').textContent=qCount;
  document.getElementById('totalScore').textContent=totalScore+' pts';
  document.getElementById('homeScore').textContent=totalScore;
}
